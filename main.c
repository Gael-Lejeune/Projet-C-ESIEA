  ...
  if (monElementTrain->train.direction == 'e') {
    ...
    switch (monElementTrain->train.etat){
      ...
      for (int i = monElementTrain->train.posx; i >= 0; i--) {
        ...
        //Afficher les caractères du train
      ...
      monElementTrain->train.posx++; //Incrémente la position du train
      ...
      if (tempsAQuai - 5 == monElementTrain->train.tempsAQuai) {
        ...
        //Si le train part dans 10, fermer les portes
        for (int i = monElementTrain->train.posx; i >= 0; i--) {
          if (monElementTrain->train.custom[2][monElementTrain->train.longueur - i] == 'f') {
        ...
        if (vitesse == 1) {
          system(SONDEPART); //Son du train qui part
        }
      }
      ...
      if(monElementTrain->train.posx == monElementTrain->train.longueur + LONGUEUR+1){
        ...
        supp_train_debut(&maListe); //Le supprimer de la liste
        ajout_train_debut(&maListe, init_train(train, 'e', 180 +  rand() % 500)); //Ajouter un autre train à la place
      }
      break;
    }
  }
    ...
    switch (monElementTrain->train.etat){
      ...
      if (monElementTrain->train.tempsAttente == 0){
        ...
        monElementTrain->train.tempsAttente--; //Décrementer le temps d'attente restant
      }
      ...
      monElementTrain->train.tempsAQuai--; //Décrementer le temps à quai
      break;
      ...
      if(monElementTrain->train.posx == -(monElementTrain->train.longueur+1)){
        supp_train_fin(&maListe);
        ajout_train_fin(&maListe, init_train(train, 'o', 180 +  rand() % 500));
      }
      break;
    }
  }
...
int main() {
  ...
  while(1){
    ...
    printf("\033[%d;%dH", 40, 0); //Affichage de la flèche de sélection
    while(1){
      c = key_pressed();
      if (c == 'q') {
        ...
        system("killall play 2> /dev/null &"); //Stopper le son
        return 0;
    ...
    if (option == 1) {
      ...
      LISTE maListeTrain; init_liste_train(&maListeTrain); //Création de la liste de train
      ajout_train_debut(&maListeTrain,init_train(train, 'o', 180 + rand() % 200));
      ajout_train_debut(&maListeTrain,init_train(train, 'e', 180 + rand() % 200)); //Ajout des deux trains
      ...
      while(1){
        if(time == 500){
          c = key_pressed();
          if (c == 'm') {
            //Quitter la simulation pour retourner au menu
            system("killall play 2> /dev/null &");
            break;
          }
          else if (c == '+') {
          ...
          if (maListeTrain.dernier->train.tempsAttente%60 == 59) {
            ...
            if(maListeTrain.dernier->train.tempsAttente/60 == 0) {
              ...
              printf("\033[%d;%dH%dm\n", 2, 2, maListeTrain.dernier->train.tempsAttente/60);
              printf("\033[%d;%dH%dm\n", 2, 122, maListeTrain.dernier->train.tempsAttente/60); //Afficher le temps d'attente restant aux panneaux
            }
            else {
              printf("\033[%d;%dH%dm \n", 2, 2, maListeTrain.dernier->train.tempsAttente/60);
              printf("\033[%d;%dH%dm \n", 2, 122, maListeTrain.dernier->train.tempsAttente/60); //Afficher le temps d'attente restant aux panneaux
          ...
          if (maListeTrain.premier->train.tempsAttente%60 == 59) {
            ...
            if(maListeTrain.premier->train.tempsAttente/60 == 0) {
              ...
              printf("\033[%d;%dH%dm\n", 28, 2, maListeTrain.premier->train.tempsAttente/60);
              printf("\033[%d;%dH%dm\n", 28, 122, maListeTrain.premier->train.tempsAttente/60); //Afficher le temps d'attente restant aux panneaux
            }
            else {
              printf("\033[%d;%dH%dm \n", 28, 2, maListeTrain.premier->train.tempsAttente/60);
              printf("\033[%d;%dH%dm \n", 28, 122, maListeTrain.premier->train.tempsAttente/60); //Afficher le temps d'attente restant aux panneaux
      ...
      LISTEV maListeV; init_liste_voy(&maListeV); //Initialisation de la liste de voyageurs

      ajout_voy_debut(&maListeV, init_voyageurInd('*', magare, 1, 0)); //Ajout d'un voyageur dans la liste
      ...
      LISTE maListeTrain; init_liste_train(&maListeTrain); //Liste de train
      ajout_train_debut(&maListeTrain,init_train(train, 'o', 180 + rand() % 500));
      ajout_train_debut(&maListeTrain,init_train(train, 'e', 180 + rand() % 500)); //Ajout des deux trains dans la liste
      printf("\033[%d;%dH%dm\n", 2, 2, maListeTrain.dernier->train.tempsAttente/60);
      printf("\033[%d;%dH%dm\n", 2, 122, maListeTrain.dernier->train.tempsAttente/60);
      printf("\033[%d;%dH%dm\n", 28, 2, maListeTrain.premier->train.tempsAttente/60);
        ...
        if(timeTrain == 500){
          ...
          if (movPlayer != 0) {
            ...
            if((monvoyageur->posX == 0 && monvoyageur->posY < 62 && movPlayer == 'z') || (monvoyageur->posY == 124 && movPlayer == 'd') ){
              ...
              monvoyageur->posX = 24;
              monvoyageur->posY = 0;
              printf("\033[%d;%dHVous venez de changer de quai.",39, 80);
              printf("\033[%d;%dHSortez de la gare à droite ou montez dans un train.",40, 80);
            }
              ...
              monvoyageur->posY = 93;
              printf("\033[%d;%dHVous venez de changer de quai.",39, 80);
              printf("\033[%d;%dHSortez de la gare à gauche ou montez dans un train.",40, 80);
            }
            mvt_voyageur(monvoyageur, magare, movPlayer);
          ...
          if (monElementTrain->train.etat == 's') {
            ...
            if(monElementTrain->train.vide == 'v' && monElementTrain->train.direction == 'o'){
              //Si le train est vide et va vers l'ouest
              monElementTrain->train.vide = 'p'; //Passer le train à "plein"
              int nbvoytrain = rand()%16+2; //Choix d'un nombre aléatoire de voyageurs à mettre dans le train
              for(int i = nbvoytrain; i>0; i--){
                //Créer les voyageurs
                ajout_voy_debut(&maListeV, init_voyageurInd('*', magare, 0, 'h'));
              }
            }
              ...
              for(int i = nbvoytrain; i>0; i--){
                ajout_voy_debut(&maListeV, init_voyageurInd('*', magare, 0, 'b'));
              }
            }
            if(monElementTrain->train.tempsAQuai > 110 && (monElementTrain->train.tempsAQuai%5)==0){
              ...
              if(monElementTrain->train.direction == 'o'){
                ...
                ajout_voy_debut(&maListeV, init_voyageurInd('*', magare, 0, 'h')); //Ajouter un voyageur sortant du train en haut
              }
              else {
                //S'il va vers l'est
                ajout_voy_debut(&maListeV, init_voyageurInd('*', magare, 0, 'b')); //Ajouter un voyageur sortant du train en bas
              }
            }
            ...
            if (monElementTrain->train.direction == 'e' && monElementTrain->train.tempsAQuai > 10) {
              ...
              while (monElementVoy) {
                ...
                if (monElementVoy->voyageur->quai == 'b' && monElementVoy->voyageur->destinationX != monElementTrain->train.lPortes) {
                  ...
                  monElementVoy->voyageur->destinationX = monElementTrain->train.lPortes; //Abscisse des portes
            ...
            while(monElementVoy){
              ...
              if (monElementVoy->voyageur->destinationX == monElementTrain->train.lPortes) {
                ...
                if (monElementVoy->voyageur->quai == 'h') {
                  //Definir au voyageur une destination aléatoire sur le quai où il attend
        ...
        if(timeVoyageur == 5000){
          ...
          while (monElementVoy) {
            ...
            if (monElementVoy->voyageur->destinationX == -1 && monElementVoy->voyageur->posX < 6 && monElementVoy->voyageur->posX > 2) {
              //Si le voyageur a certaines positions
              mult = 5;
            }
            ...
            if (monElementVoy->voyageur->tempsAttente == 0) {
              ...
              if (abs(monElementVoy->voyageur->posX-monElementVoy->voyageur->destinationX)*mult > abs(monElementVoy->voyageur->posY-monElementVoy->voyageur->destinationY)) {
                ...
                if (monElementVoy->voyageur->posX > monElementVoy->voyageur->destinationX) {
                  //Opération sur l'axe X
                  if (mvt_voyageur(monElementVoy->voyageur, magare, 'z') == 0) {
                    //Si le voyageur ne peut aller vers le haut
                    mvt_voyageur(monElementVoy->voyageur, magare, 'q'); //Aller à gauche
                  }
                }
                else if(monElementVoy->voyageur->posX < monElementVoy->voyageur->destinationX){
                  if(mvt_voyageur(monElementVoy->voyageur, magare, 's') == 0){
                    mvt_voyageur(monElementVoy->voyageur, magare, 'q');
                  }
                }
              }
              else {
                if (monElementVoy->voyageur->posY < monElementVoy->voyageur->destinationY) {
                  if(mvt_voyageur(monElementVoy->voyageur, magare, 'd') == 0 && mvt_voyageur(monElementVoy->voyageur, magare, 's') == 0){
                    mvt_voyageur(monElementVoy->voyageur, magare, 'z');
                  }
                }
                else if(monElementVoy->voyageur->posY > monElementVoy->voyageur->destinationY){
                  if(mvt_voyageur(monElementVoy->voyageur, magare, 'q') == 0 && mvt_voyageur(monElementVoy->voyageur, magare, 's') == 0){
                    mvt_voyageur(monElementVoy->voyageur, magare, 'z');
                  }
                }
              }
              if(monElementVoy->voyageur->carpos == 'd' || monElementVoy->voyageur->carpos == 'f'){
                ...
                supp_voy(&maListeV, monElementVoy); //Supprimer le voyageur
              }
            }
            else if (monElementVoy->voyageur->tempsAttente == 1) {
              //Si le voyageur a un temps d'attente égal à 1
              ajout_voy_debut(&maListeV, init_voyageurInd('*', magare, monElementVoy->voyageur->tempsAttente, 0)); //Ajouter une nouveau voyageur
