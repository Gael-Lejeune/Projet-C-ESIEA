...
int main() {
    ...
    while(1){
        ...
        if (option == 1) { //Si le mode choisi est le premier
            ...
            while(1){
                if(timeTrain == 500){
                    if (movPlayer == 'k') { //Si la touche k est préssée
                        break; //Fin de la simulation et retour au menu
                    }
                    else if (movPlayer == '+') { //Acceleration de la simulation
                        vitesse +=1;
                        printf("\033[%d;%dH%s%d  \n", 32, 80, "x", vitesse); //Affichage du multiplicateur de vitesse
                    }
                    else if (movPlayer == '-' && vitesse > 1) { //Deceleration de la simulation
                        vitesse -=1;
                        printf("\033[%d;%dH%s%d  \n", 32, 80, "x", vitesse);
                    }
                    movPlayer = key_pressed(); //Touche préssée
                    if (movPlayer != 0) { //Si unt touche est préssée
                    ...
                    if (maListeTrain.dernier->train.tempsAttente%60 == 59) { //Affichage des temps d'attente (voir plus haut)
                    ...
                    if (monElementTrain->train.etat == 's') { //Si le train est stationné
                        ...
                        else if(monElementTrain->train.vide == 'v' && monElementTrain->train.direction == 'e'){ //Si le train est vide et va vers l'est
                        ...
                        if(monElementTrain->train.tempsAQuai > 110 && (monElementTrain->train.tempsAQuai%5)==0){ //Si le train est à quai depuis peu et que son temps d'attente est divisible par 5
