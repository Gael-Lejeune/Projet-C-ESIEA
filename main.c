  ...
  if (monElementTrain->train.direction == 'e') {
    ...
    switch (monElementTrain->train.etat){
      ...
      if (monElementTrain->train.tempsAttente == 0){
        ...
        if (vitesse == 1)
        system("play -q --no-show-progres -v 0.08 sound/freinage_metro2.mp3 &");
      }
      else{
        //Sinon
      ...
      if (tempsAQuai - 5 == monElementTrain->train.tempsAQuai) {
        ...
        if (vitesse == 1) {
          system("play -q --no-show-progres -v 0.08 sound/porte-metro.mp3 &"); //Son des portes qui se ferment
        }
      }
        ...
        if (vitesse == 1) {
          system("play -q --no-show-progres -v 0.08 sound/depart_metro.mp3 &"); //Son des portes qui se ferment
        }
      }
    ...
    switch (monElementTrain->train.etat){
      case 'd': //dehors
      if (monElementTrain->train.tempsAttente == 0){ //Si le train a assez attendu
      ...
      if(monElementTrain->train.posx == 5){ //Si le train atteint sa position d'arret
      monElementTrain->train.etat = 's'; //Passer à stationné
    }
    break;
    case 's' : //Stationné
    if (tempsAQuai - 5 == monElementTrain->train.tempsAQuai) { //Si le train est la depui peu
    ...
    else if (monElementTrain->train.tempsAQuai-10 == 0) { //Si part bientot
    ...
    else if (monElementTrain->train.tempsAQuai == 0) { //Si le train a assez attendu à quai
...
int main() {
  ...
  while(1){
    ...
    while(1){
      ...
      else if (c == ' '){ //Demarrer la simulation
        break;
      }
    ...
    if (option == 1) { //Si le mode choisi est le premier
      ...
      printf("\033[%d;%dH%s\n", 31, 2, "        (o o)");
      printf("\033[%d;%dH%s\n", 32, 2, "+----oOO-{_}-OOo------------------------------+");
      printf("\033[%d;%dH%s\n", 33, 2, "|   Commandes possibles :                     |");
      printf("\033[%d;%dH%s\n", 34, 2, "|   « M » -> menu                             |");
      printf("\033[%d;%dH%s\n", 35, 2, "|   « + » -> accélère la simulation           |");
      printf("\033[%d;%dH%s\n", 36, 2, "|   « - » -> décélère la simulation           |");
      printf("\033[%d;%dH%s\n", 37, 2, "+---------------------------------------------+");


      printf("\033[%d;%dH%s\n", 33, 80, "+--------------------------------+");
      printf("\033[%d;%dH%s\n", 34, 80, "|   Vitesse de la simulation :   |");
      printf("\033[%d;%dH|   x1                           |\n", 35, 80); //Affichage des commandes possibles
      printf("\033[%d;%dH%s\n", 36, 80, "+--------------------------------+");
      ...
      while(1){
        if(time == 500){
          c = key_pressed();
          if (c == 'm') { //Quitter la simulation pour retourner au menu
            break;
          ...
          if (maListeTrain.dernier->train.tempsAttente%60 == 59) { //Si le nombre de secondes est égal à 59 au deuxieme train
            if(maListeTrain.dernier->train.tempsAttente/60 == 0) { //Si il reste moins d'une minute au deuxieme train
        ...
        if (maListeTrain.premier->train.tempsAttente%60 == 59) {//Si le nombre de secondes est égal à 59 au premier train
          if(maListeTrain.premier->train.tempsAttente/60 == 0) {//Si il reste moins d'une minute au premier train
      ...
      if (monElementTrain->suivant == NULL) { //Si le train est le dernier de la liste
...
else if (option == 2){ //Si le mode 2 est choisi
  ...
  printf("\033[%d;%dH%s\n", 35, 2, "|   « + » -> accélère la simulation           |");
  printf("\033[%d;%dH%s\n", 36, 2, "|   « - » -> décélère la simulation           |");
  printf("\033[%d;%dH%s\n", 37, 2, "|   « Z » -> le voyageur monte                |");
  printf("\033[%d;%dH%s\n", 38, 2, "|   « S » -> le voyageur descend              |");
  printf("\033[%d;%dH%s\n", 39, 2, "|   « D » -> le voyageur va à droite          |");
  printf("\033[%d;%dH%s\n", 40, 2, "|   « Q » -> le voyageur va à gauche          |");
  printf("\033[%d;%dH%s\n", 41, 2, "+---------------------------------------------+");
  ...
  while(1){
    if(timeTrain == 500){
      if (movPlayer == 'm') { //Si la touche m est préssée
        break; //Fin de la simulation et retour au menu
      }
      else if (movPlayer == '+') { //Accélération de la simulation
        vitesse +=1;
        printf("\033[%d;%dH%s%d  \n", 35, 84, "x", vitesse); //Affichage du multiplicateur de vitesse
      }
      else if (movPlayer == '-' && vitesse > 1) { //Décélération de la simulation
        vitesse -=1;
        printf("\033[%d;%dH%s%d  \n", 35, 84, "x", vitesse);
      }
      movPlayer = key_pressed(); //Touche préssée
      if (movPlayer != 0) { //Si une touche est préssée
        ...
        if (monvoyageur->carpos == 'd' || monvoyageur->carpos == 'f') { //Si le voyageur entre dans un train
          system("clear");
      ...
      if (monElementTrain->train.etat == 's') { //Si le train est stationné
        if(monElementTrain->train.vide == 'v' && monElementTrain->train.direction == 'o'){ //Si letrain est vide et va vers l'ouest
        ...
        for(int i = nbvoytrain; i>0; i--){ //Créer les voyageurs
    ...
    if(monElementTrain->train.direction == 'o'){ //S'il va vers l'ouest
...
if (monElementTrain->train.direction == 'e' && monElementTrain->train.tempsAQuai > 10) { //Si le train a ouvert ses portes
  while (monElementVoy) { //Pour tout les voyageurs
    if (monElementVoy->voyageur->quai == 'b' && monElementVoy->voyageur->destinationX != monElementTrain->train.lPortes) { //Si le voyageur est en bas et a une destination autre que les portes
      ...
      for (int i = 0; i <= LONGUEUR; i++) { //Pour la longueur de la matrice
        if (magare.custom[monElementTrain->train.lPortes][i] == 'f' && abs(i - monElementVoy->voyageur->posY) < abs(monElementVoy->voyageur->destinationY - monElementVoy->voyageur->posY) ) { //Si on observe une destination/porte plus proche que l'actuelle, remplacer
...
else if(monElementTrain->train.direction == 'o' && monElementTrain->train.tempsAQuai > 10){ //Pareil pour le train allant vers l'ouest
while (monElementVoy) {
...
else if (monElementTrain->train.etat == 'p') {// Si le train est partant
  monElementVoy = maListeV.premier;
  while(monElementVoy){ //Pour tout les voyageurs
    if (monElementVoy->voyageur->destinationX == monElementTrain->train.lPortes) { //Si la destination du voyageur est une porte du train
      if (monElementVoy->voyageur->quai == 'h') { //Definir au voyageur une destination aléatoire sur le quai ou il attend
...
if (monElementTrain->suivant == NULL) { //Si le train actuel est le dernier de la liste
  monElementTrain = maListeTrain.premier; //Selectionner le premier
}
else{
  monElementTrain = maListeTrain.dernier; //Selectionner le suivant
...
if(timeVoyageur == 5000){
  ...
  int posX;
  while (monElementVoy) { //Pour tout les voyageurs
    if (monElementVoy->voyageur->destinationX == -1 && monElementVoy->voyageur->posX < 6 && monElementVoy->voyageur->posX > 2) { //Si le voyageur a certaines position
      mult = 5;
    }
    ...
    if (monElementVoy->voyageur->tempsAttente == 0) { //Si le voyageur a assez attendu hors de la gare
      if (abs(monElementVoy->voyageur->posX-monElementVoy->voyageur->destinationX)*mult > abs(monElementVoy->voyageur->posY-monElementVoy->voyageur->destinationY)) { //Comparaison entre la destination et la position
        if (monElementVoy->voyageur->posX > monElementVoy->voyageur->destinationX) { //Oppération sur l'axe X
        if (mvtVoy(monElementVoy->voyageur, magare, 'z') == 0) { //Si el voyageur ne peut aller vers el haut
    ...
    if(monElementVoy->voyageur->carpos == 'd' || monElementVoy->voyageur->carpos == 'f'){ //Si le voyageur entre dans une porte de train
      suppV(&maListeV, monElementVoy); //Supprimer le voyageur
    }
  }
  else if (monElementVoy->voyageur->tempsAttente == 1) { //Si le voyageur a un temps d'attente égal à 1
